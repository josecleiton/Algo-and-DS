  ...
  public:
   void print(void) {
      ...
      for (auto vertex : Graph<ValueType>::adj_list) {
         cout << vertex.first << ": ";
         for (auto edge : vertex.second) cout << edge << " -> ";
         cout << "/" << endl;
      }
   }
};
  ...
  public:
   void popVertex(ValueType v) {
      ...
      for (auto& vertex : Graph<ValueType>::adj_list) {
         ...
         if (toFind != vertex.second.end()) vertex.second.erase(toFind);
      }
   }
  ...
  public:
   ...
   map<ValueType, ValueType> dijkstra(ValueType start, ValueType finish) {
      ...
      while (!priorityq.empty()) {
         ...
         for (auto& nbh :
              weighted_graph<ValueType>::adj_list[smallest.second]) {
   ...
   pair<map<ValueType, int>, map<ValueType, ValueType>> bellmanFord(ValueType start) {
      map<ValueType, int> distances;
      map<ValueType, ValueType> previous;
      for (auto vertex : weighted_graph<ValueType>::adj_list) {
         previous[vertex.first];
