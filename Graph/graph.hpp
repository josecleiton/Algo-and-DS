...
template <typename ValueType>
class undirected_graph: public Graph<ValueType> {
protected:
    // TRAVERSAL UTILS
...
public:
    ...
    void popEdge(ValueType v1, ValueType v2){
        if(!this->isVertex(v1) or !this->isVertex(v2)) return;
        auto find_in_v1 = find(Graph<ValueType>::adj_list[v1].begin(), Graph<ValueType>::adj_list[v1].end(), v2);
        auto find_in_v2 = find(Graph<ValueType>::adj_list[v2].begin(), Graph<ValueType>::adj_list[v2].end(), v1);
        if(find_in_v1 != Graph<ValueType>::adj_list[v1].end()
        and find_in_v2 != Graph<ValueType>::adj_list[v2].end()){
...
public:
    void dfs(ValueType key){
        if(Graph<ValueType>::adj_list.find(key) == Graph<ValueType>::adj_list.end()) return;
        visited.clear();
        results.clear();
...
public:
    struct Node{
        ValueType data;
        int weight;
        Node(ValueType _data = ValueType(), int _w = 0):
            data(_data), weight(_w){}
    };
public:
    void addVertex(ValueType v){
        adj_list[v];
...
public:
    ...
    void addEdge(ValueType v1, ValueType v2, int weight = 0){
        weighted_graph<ValueType>::adj_list[v1].push_back(typename weighted_graph<ValueType>::Node(v2, weight));
        weighted_graph<ValueType>::adj_list[v2].push_back(typename weighted_graph<ValueType>::Node(v1, weight));
    }
    ...
    void dfsUtil(ValueType& root){
        ...
        for(auto& nbh: weighted_graph<ValueType>::adj_list[root])
            if(!visited[nbh.data]) dfsUtil(nbh.data);
    }
